...
class Agent(object):
    ...
    def __init__(self, input_dim, output_dim, hidden_dims=[8, 8]):
        """Gym Playing Agent
    ...
    def __build_network(self, input_dim, output_dim, hidden_dims=[8, 8]):
...
def compute_discounted_R(R, discount_rate=.99):
    ...
    discounted_r_std = discounted_r.std()
    if discounted_r_std == 0:
        discounted_r = 0.00001
    else:
        discounted_r -= discounted_r.mean() / discounted_r_std
...
def main():
    try:
        ...
        agent = Agent(input_dim, output_dim, [4, 4])
        
        for episode in range(5000):  
            reward, info = run_episode(env, agent, verbose = 0)
            print str(episode), str(reward), info['Steps']
